package GEDCOM::ReportEngine::Record::Date::Interval;

=head1 GEDCOM::ReportEngine::Record::Date::Interval
...
sub as_string {
    ...
    if ( $self->{date1} && $self->{date2} ) {
        ...
        $t = sprintf( "efter %s", $self->$sub( @{ $self->{date1} } ) );
    }
    elsif ( $self->{date2} ) {
        $t = sprintf( "före %s", $self->$sub( @{ $self->{date2} } ) );
    }
    else {
