...
=head1 The Gedcom::Report::Record::Date package

=over 8

=cut

package Gedcom::Report::Record::Date;
use base qw(Gedcom::Report::Record);
use strict;
use warnings;
use utf8;
use Gedcom::Report::Locale;
use Gedcom::Report::Record::Date::Range;
use Gedcom::Report::Record::Date::Interval;
...
sub _parsedate {
    ...
    $date =~ m/^((\d+)\s+)?((JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)\s+)?(\d{4})$/i
...
sub prefix {
    my $self = shift;
    my $t = '';
    $t .= T( "estimated " ) if ($self->{estimated});
    $t .= T( "calculated " ) if ($self->{calculated});
    $t .= T( "about " ) if ($self->{approximated});
    return $t;
}
